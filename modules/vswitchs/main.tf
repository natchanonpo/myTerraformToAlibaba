resource "alicloud_vswitch" "ecs_vswitchs" {
  ...
  vswitch_name = "${var.names_prefix}-SNT${count.index + 1}-${var.ecs_names_suffix}"
...
resource "alicloud_vswitch" "rds_vswitchs" {
  ...
  vswitch_name = "${var.names_prefix}-SNT${count.index + length(alicloud_vswitch.ecs_vswitchs) + 1}-${var.rds_names_suffix}"
...
resource "alicloud_vswitch" "kafka_vswitchs" {
  ...
  vswitch_name = "${var.names_prefix}-SNT${count.index + length(alicloud_vswitch.ecs_vswitchs) + length(alicloud_vswitch.rds_vswitchs) + 1}-${var.kafka_names_suffix}"
