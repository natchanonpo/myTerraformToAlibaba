locals {
  ...
  db_external_full_names = flatten([
    for env in var.allowed_external_db_envs : [
      for db_name in var.db_names :
      format(var.db_name_template, env, index(var.db_names, db_name) + 1, db_name)
    ]
  ])
...
resource "alicloud_db_account_privilege" "db_xom_editor_rights" {
  ...
  db_names     = alicloud_db_database.*.name
  privilege    = "ReadWrite"
}
...
resource "alicloud_db_account_privilege" "db_external_editor_rights" {
  ...
  db_names     = matchkeys(alicloud_db_database.*.name, alicloud_db_database.*.name, local.db_external_full_names)
  privilege    = "ReadWrite"
}
